/* ==========================================================
 * 心理咨询室·日记 (psych-counseling-room) v3.1.0
 *
 * v3.1 变更：
 * - 日记支持标题（可编辑，列表和详情显示）
 * - 自定义字体（本地字体文件上传 / 字体链接）
 * - 所有 emoji 替换为统一风格 SVG 图标
 * - 返回 Roche 本体按钮
 * ========================================================== */
...
(function () {
  ...
  const K_FONT_CONFIG = "diary-font-config";

  /* ---- 心情（SVG 图标，扁平化风格） ---- */
  const MOODS = [
    { key:"happy",    icon:"smile",       label:"开心", color:"#7EC8A0" },
    { key:"calm",     icon:"leaf",        label:"平静", color:"#8ECDE0" },
    { key:"grateful", icon:"heart",       label:"感恩", color:"#F0A8B8" },
    { key:"sad",      icon:"cloud-rain",  label:"难过", color:"#8BAEE0" },
    { key:"angry",    icon:"flame",       label:"愤怒", color:"#E0A0A0" },
    { key:"anxious",  icon:"alert",       label:"焦虑", color:"#E0B896" },
    { key:"tired",    icon:"moon",        label:"疲惫", color:"#B0C4DE" },
    { key:"confused", icon:"help",        label:"困惑", color:"#C8B0D8" },
    { key:"fear",     icon:"shield",      label:"恐惧", color:"#A0A8D8" },
    { key:"numb",     icon:"minus",       label:"无力", color:"#B8B8C0" }
  ];
  ...
  function weatherCodeToIconKey(code) {
  ...
  const MANUAL_WEATHERS = [
    { iconKey:"sun",desc:"晴" },{ iconKey:"cloud-sun",desc:"多云" },{ iconKey:"cloud",desc:"阴" },{ iconKey:"cloud-rain",desc:"雨" },
    { iconKey:"cloud-lightning",desc:"雷雨" },{ iconKey:"cloud-snow",desc:"雪" },{ iconKey:"fog",desc:"雾" },{ iconKey:"wind",desc:"恶劣" }
  ];
  ...
  /* =========================================================
   *  SVG 图标库 — 统一 stroke 风格
   * ========================================================= */
  function svg(paths, sz) {
    sz = sz || 20;
    return `<svg width="${sz}" height="${sz}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>`;
  }
  const ICO = {
    back: svg('<polyline points="15 18 9 12 15 6"></polyline>'),
    home: svg('<path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"></polyline>',18),
    book: svg('<path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>',22),
    ...
    save: svg('<path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/>',18),
    ...
    heart: svg('<path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>',16),
    reset: svg('<path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/>',14),
    chevron: svg('<polyline points="6 9 12 15 18 9"></polyline>',16),
    cloud: svg('<path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z"/>',18),
    image: svg('<rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>',18),
    close: svg('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>',16),
    calendar: svg('<rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>',16),
    clock: svg('<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>',16),
    chart: svg('<line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/>',16),
    chat: svg('<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>',14),
    ...
    "cloud-sun": svg('<path d="M12 2v2M5.6 5.6l1.4 1.4M2 12h2M20 12h2M17 7l1.4-1.4"/><circle cx="8" cy="8" r="3"/><path d="M18 22H8a4 4 0 0 1 0-8 6 6 0 0 1 11 2 3 3 0 0 1-1 6z"/>',18),
    "cloud": svg('<path d="M18 10h-1.26A8 8 0 1 0 9 20h9a5 5 0 0 0 0-10z"/>',18),
    "fog": svg('<line x1="3" y1="8" x2="21" y2="8"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="16" x2="21" y2="16"/><line x1="3" y1="20" x2="17" y2="20"/>',18),
    "cloud-rain": svg('<line x1="8" y1="19" x2="8" y2="21"/><line x1="8" y1="13" x2="8" y2="15"/><line x1="16" y1="19" x2="16" y2="21"/><line x1="16" y1="13" x2="16" y2="15"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="12" y1="15" x2="12" y2="17"/><path d="M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25"/>',18),
    "cloud-snow": svg('<path d="M20 17.58A5 5 0 0 0 18 8h-1.26A8 8 0 1 0 4 16.25"/><line x1="8" y1="16" x2="8.01" y2="16"/><line x1="8" y1="20" x2="8.01" y2="20"/><line x1="12" y1="18" x2="12.01" y2="18"/><line x1="12" y1="22" x2="12.01" y2="22"/><line x1="16" y1="16" x2="16.01" y2="16"/><line x1="16" y1="20" x2="16.01" y2="20"/>',18),
    "cloud-lightning": svg('<path d="M19 16.9A5 5 0 0 0 18 7h-1.26a8 8 0 1 0-11.62 9"/><polyline points="13 11 9 17 15 17 11 23"/>',18),
    "wind": svg('<path d="M9.59 4.59A2 2 0 1 1 11 8H2m10.59 11.41A2 2 0 1 0 14 16H2m15.73-8.27A2.5 2.5 0 1 1 19.5 12H2"/>',18),
    /* 心情图标 */
    "smile": svg('<circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/>',22),
    "leaf": svg('<path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 4.48 19.2 2.96c1.4 9.3-3.6 15.65-8.2 17.04"/><path d="M2 21c0-3 1.85-5.36 5.08-6"/>',22),
    "cloud-rain-mood": svg('<line x1="8" y1="19" x2="8" y2="21"/><line x1="8" y1="13" x2="8" y2="15"/><line x1="16" y1="19" x2="16" y2="21"/><line x1="16" y1="13" x2="16" y2="15"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="12" y1="15" x2="12" y2="17"/><path d="M20 16.58A5 5 0 0 0 18 7h-1.26A8 8 0 1 0 4 15.25"/>',22),
    "flame": svg('<path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/>',22),
    "alert": svg('<path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>',22),
    "moon": svg('<path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/>',22),
    "help": svg('<circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/>',22),
    "shield": svg('<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>',22),
    "minus": svg('<circle cx="12" cy="12" r="10"/><line x1="8" y1="12" x2="16" y2="12"/>',22),
    /* 功能图标 */
    "edit": svg('<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>',16),
    "type": svg('<polyline points="4 7 4 4 20 4 20 7"/><line x1="9" y1="20" x2="15" y2="20"/><line x1="12" y1="4" x2="12" y2="20"/>',18),
    "upload": svg('<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/>',18),
    "link": svg('<path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/>',18),
    "brain": svg('<path d="M9.5 2A2.5 2.5 0 0 1 12 4.5v15a2.5 2.5 0 0 1-4.96.44 2.5 2.5 0 0 1-2.96-3.08 3 3 0 0 1-.34-5.58 2.5 2.5 0 0 1 1.32-4.24 2.5 2.5 0 0 1 1.98-3A2.5 2.5 0 0 1 9.5 2z"/><path d="M14.5 2A2.5 2.5 0 0 0 12 4.5v15a2.5 2.5 0 0 0 4.96.44 2.5 2.5 0 0 0 2.96-3.08 3 3 0 0 0 .34-5.58 2.5 2.5 0 0 0-1.32-4.24 2.5 2.5 0 0 0-1.98-3A2.5 2.5 0 0 0 14.5 2z"/>',18),
    "wave": svg('<path d="M2 12c2-3 4-3 6 0s4 3 6 0 4-3 6 0"/><path d="M2 18c2-3 4-3 6 0s4 3 6 0 4-3 6 0"/><path d="M2 6c2-3 4-3 6 0s4 3 6 0 4-3 6 0"/>',18),
    "users": svg('<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>',18),
    "dumbbell": svg('<path d="M6.5 6.5l11 11"/><path d="M21 21l-1-1"/><path d="M3 3l1 1"/><path d="M18 22l4-4"/><path d="M2 6l4-4"/><path d="M3 10l7-7"/><path d="M14 21l7-7"/>',18),
    "trending": svg('<polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/>',18),
    "note": svg('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="14" y2="17"/>',18),
    "loading": svg('<path d="M21 12a9 9 0 1 1-6.219-8.56"/>',18),
    "gauge": svg('<path d="M3 12a9 9 0 0 1 9-9 9 9 0 0 1 6 2.3L21 8"/><path d="M21 3v5h-5"/>',16)
  };

  function moodIcon(key, sz) {
    const m = moodByKey(key);
    const iconKey = m.icon === "heart" ? "heart" : m.icon;
    return ICO[iconKey] ? ICO[iconKey] : ICO["smile"];
  }

  /* ---- 提示词系统 ---- */
  const PROMPT_CATEGORIES = [
    {
      id:"counselor", title:"将日记发给 AI", iconKey:"send",
    ...
    {
      id:"profile", title:"成长档案", iconKey:"sprout",
  ...
  function fmtTokens(n){return n<1000?n+" tokens":(n/1000).toFixed(1)+"k tokens";}
  ...
  function fileToCompressedBase64(file,maxW,quality) {
    return new Promise((resolve,reject)=>{
      ...
      reader.onload=(e)=>{
        const img=new Image();
        img.onload=()=>{
          const mw=maxW||1024;let w=img.width,h=img.height;
  ...
  async function fetchWeather() {
    try {
      const pos=await new Promise((res,rej)=>{if(!navigator.geolocation)return rej(new Error("no geo"));navigator.geolocation.getCurrentPosition(res,rej,{timeout:6000,enableHighAccuracy:false});});
      const lat=pos.coords.latitude.toFixed(2),lon=pos.coords.longitude.toFixed(2);
      const res=await fetch("https://api.open-meteo.com/v1/forecast?latitude="+lat+"&longitude="+lon+"&current_weather=true");
      const data=await res.json();
      if(data&&data.current_weather){const cw=data.current_weather;return{temp:Math.round(cw.temperature),code:cw.weathercode,desc:weatherCodeToDesc(cw.weathercode),iconKey:weatherCodeToIconKey(cw.weathercode),source:"auto"};}
    } catch(e){}
    try {
      const res=await fetch("https://wttr.in/?format=j1",{headers:{"Accept":"application/json"}});
      if(res.ok){const data=await res.json();const cur=data.current_condition&&data.current_condition[0];const area=data.nearest_area&&data.nearest_area[0];
        if(cur){const code=parseInt(cur.weatherCode);return{temp:parseInt(cur.temp_C),code,desc:weatherCodeToDesc(code)||(cur.weatherDesc&&cur.weatherDesc[0]&&cur.weatherDesc[0].value)||"未知",iconKey:weatherCodeToIconKey(code),city:area?(area.areaName&&area.areaName[0]&&area.areaName[0].value):"",source:"auto"};}}
    } catch(e){}
    return null;
  }
  ...
  function applyApiConfig(opts,cfg){if(!cfg)return opts;if(cfg.provider)opts.provider=cfg.provider;if(cfg.model)opts.model=cfg.model;if(cfg.endpoint)opts.endpoint=cfg.endpoint;if(cfg.apiKey)opts.apiKey=cfg.apiKey;return opts;}
  async function fetchModels(endpoint,apiKey){
    if(!endpoint)return[];
    try{
      const url=endpoint.replace(/\/+$/,"")+"/models";const headers={"Content-Type":"application/json"};if(apiKey)headers["Authorization"]="Bearer "+apiKey;
      const res=await fetch(url,{headers});if(!res.ok)return[];const data=await res.json();
  ...
  /* ---- 字体配置 ---- */
  async function getFontConfig(roche){return await getJSON(roche,K_FONT_CONFIG,{type:"default"});}
  function applyFont(root,fontCfg){
    if(!fontCfg||fontCfg.type==="default"||!fontCfg.value)return;
    let fontId="dpr-custom-font";
    let existing=document.getElementById(fontId);
    if(fontCfg.type==="url"){
      if(!existing){existing=document.createElement("style");existing.id=fontId;document.head.appendChild(existing);}
      existing.textContent="@font-face{font-family:'DprCustom';src:url('"+fontCfg.value+"') format('woff2'),url('"+fontCfg.value+"') format('woff'),url('"+fontCfg.value+"') format('truetype');}";
      root.style.setProperty("--diary-font","'DprCustom', serif");
    } else if(fontCfg.type==="base64"){
      if(!existing){existing=document.createElement("style");existing.id=fontId;document.head.appendChild(existing);}
      const fmt=fontCfg.format||"truetype";
      existing.textContent="@font-face{font-family:'DprCustom';src:url('"+fontCfg.value+"') format('"+fmt+"');}";
      root.style.setProperty("--diary-font","'DprCustom', serif");
    } else {
  ...
  const STYLE_TEXT=`
.dpr-app {
  --bg: #F2F6F4; --card: #FFFFFF; --card-alt: #F7FAF8;
  --primary: #5B9B8A; --primary-light: #E8F4F0; --primary-dark: #3D7A6A;
  --accent: #7EC8B8; --ink: #3A4A42; --ink-light: #6B7B73;
  --sub: #9CA8A2; --border: #E2E8E5; --danger: #D4756B;
  --danger-light: #FDF0EE; --success: #6BAA8F; --warn: #E0B870;
  --r: 12px; --t: 0.25s ease;
  --diary-font: inherit;
...
.dpr-header { display:flex; align-items:center; gap:8px; padding:10px 12px; background:var(--card); border-bottom:1px solid var(--border); min-height:52px; flex-shrink:0; }
.dpr-header-title { font-size:17px; font-weight:700; color:var(--ink); }
.dpr-header-sub { font-size:11px; color:var(--sub); }
.dpr-icon-btn { display:inline-flex; align-items:center; justify-content:center; width:36px; height:36px; border:none; background:transparent; color:var(--ink-light); cursor:pointer; border-radius:50%; transition:background var(--t); flex-shrink:0; }
.dpr-icon-btn:active { background:var(--primary-light); }

.dpr-tabbar { display:flex; background:var(--card); border-top:1px solid var(--border); flex-shrink:0; padding-bottom:env(safe-area-inset-bottom,0px); }
.dpr-tab { flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:6px 4px; font-size:11px; color:var(--sub); cursor:pointer; gap:2px; transition:color var(--t); font-weight:500; }
...
.dpr-book-cover { aspect-ratio:3/4; border-radius:4px 10px 10px 4px; display:flex; flex-direction:column; align-items:center; justify-content:center; position:relative; overflow:hidden; color:#f5f5f5; }
...
.dpr-empty-shelf-icon { margin-bottom:12px; opacity:0.3; display:flex; justify-content:center; color:var(--sub); }

.dpr-fab { position:absolute; bottom:20px; right:20px; width:56px; height:56px; border-radius:50%; background:var(--primary); color:#fff; border:none; display:flex; align-items:center; justify-content:center; cursor:pointer; z-index:15; transition:transform var(--t); }
...
.dpr-entry-card { background:var(--card); border-radius:var(--r); padding:16px; margin-bottom:12px; cursor:pointer; position:relative; transition:transform var(--t); border:1px solid var(--border); }
.dpr-entry-card:active { transform:scale(0.98); }
.dpr-entry-top { display:flex; align-items:center; gap:8px; margin-bottom:6px; }
.dpr-entry-mood { color:var(--mood-c); display:flex; align-items:center; }
...
.dpr-entry-title { font-size:15px; font-weight:700; color:var(--ink); margin-bottom:4px; line-height:1.4; }
.dpr-entry-preview { font-size:13.5px; line-height:1.7; color:var(--ink-light); overflow:hidden; text-overflow:ellipsis; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; }
...
.dpr-badge { font-size:10px; padding:2px 8px; border-radius:10px; font-weight:500; display:flex; align-items:center; gap:3px; }
...
.dpr-read-stamp { position:absolute; top:14px; right:14px; width:44px; height:44px; border:2.5px solid var(--danger); border-radius:50%; display:flex; align-items:center; justify-content:center; color:var(--danger); font-size:13px; font-weight:700; transform:rotate(-15deg); opacity:0.4; pointer-events:none; }
...
.dpr-title-input { width:100%; border:none; background:transparent; padding:8px 0 4px; font-size:20px; font-weight:700; color:var(--ink); outline:none; font-family:inherit; }
.dpr-title-input::placeholder { color:var(--border); font-weight:400; }
...
.dpr-mood-chip { flex-shrink:0; display:flex; flex-direction:column; align-items:center; gap:3px; padding:8px 10px; border-radius:12px; border:2px solid var(--border); background:var(--card-alt); cursor:pointer; transition:all var(--t); min-width:56px; color:var(--sub); }
.dpr-mood-chip.selected { border-color:var(--mood-c); background:color-mix(in srgb,var(--mood-c) 15%,var(--card)); transform:translateY(-2px); color:var(--mood-c); }
.dpr-mood-chip-icon { display:flex; align-items:center; justify-content:center; }
...
.dpr-weather-icon { color:var(--primary); display:flex; align-items:center; }
...
.dpr-weather-manual-btn { border:1px solid var(--border); background:var(--card); border-radius:8px; padding:5px 10px; font-size:12px; cursor:pointer; display:flex; align-items:center; gap:4px; color:var(--sub); transition:all var(--t); }
.dpr-weather-manual-btn.selected { background:var(--primary-light); color:var(--primary-dark); border-color:var(--primary); }
.dpr-weather-manual-btn svg { color:var(--primary); }

.dpr-editor-c { flex:1; display:flex; flex-direction:column; overflow:hidden; background:var(--card); }
.dpr-textarea { flex:1; width:100%; border:none; background:transparent; padding:12px 20px; font-size:15px; line-height:1.8; font-family:var(--diary-font); color:var(--ink); outline:none; resize:none; min-height:120px; }
...
.dpr-token-warn { color:var(--warn); font-weight:600; }
.dpr-token-ok { color:var(--primary); font-weight:600; }
...
.dpr-detail-mood { color:var(--mood-c); display:flex; align-items:center; }
.dpr-detail-date { font-size:17px; font-weight:700; color:var(--ink); }
.dpr-detail-title { font-size:20px; font-weight:700; color:var(--ink); margin-bottom:4px; line-height:1.4; }
...
.dpr-detail-content { font-size:15px; line-height:2; color:var(--ink); white-space:pre-wrap; word-break:break-word; font-family:var(--diary-font); }
...
.dpr-ai-avatar { width:32px; height:32px; border-radius:50%; background:var(--primary); display:flex; align-items:center; justify-content:center; color:#fff; flex-shrink:0; }
...
.dpr-profile-hero-icon { margin-bottom:8px; display:flex; justify-content:center; }
...
.dpr-mood-dot-icon { color:var(--mood-c); display:flex; align-items:center; }
...
.dpr-set-title { font-size:15px; font-weight:700; color:var(--ink); margin-bottom:12px; display:flex; align-items:center; gap:6px; }
...
.dpr-prompt-cat-icon { display:flex; align-items:center; }
...
/* 字体配置 */
.dpr-font-preview { font-size:16px; color:var(--ink); padding:12px 14px; background:var(--card-alt); border:1px solid var(--border); border-radius:8px; margin-bottom:10px; line-height:1.8; }
.dpr-font-type-row { display:flex; gap:8px; margin-bottom:10px; }
.dpr-font-type-btn { flex:1; border:1px solid var(--border); background:var(--card); border-radius:8px; padding:8px 12px; font-size:13px; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:4px; color:var(--sub); transition:all var(--t); font-family:inherit; }
.dpr-font-type-btn.selected { background:var(--primary-light); color:var(--primary-dark); border-color:var(--primary); }
`;
  ...
  async function mount(container, roche) {
    if (!document.getElementById(STYLE_ID)) {
      const s=document.createElement("style");s.id=STYLE_ID;s.textContent=STYLE_TEXT;document.head.appendChild(s);
    }
    const root=document.createElement("div");
    root.className="dpr-app";
    root.innerHTML=`
      <div class="dpr-header">
        <button class="dpr-icon-btn" id="dpr-home" title="返回">${ICO.home}</button>
        <button class="dpr-icon-btn" id="dpr-back" style="display:none;">${ICO.back}</button>
        <div style="flex:1;"><div class="dpr-header-title" id="dpr-htitle">我的日记</div><div class="dpr-header-sub" id="dpr-hsub">记录每一个值得被看见的瞬间</div></div>
      </div>
      <div class="dpr-content" id="dpr-content">
        <div class="dpr-view active" data-view="shelf">
          <div class="dpr-shelf-c" id="dpr-shelf"></div>
          <button class="dpr-fab" id="dpr-fab">${ICO.write}</button>
        </div>
        ...
        <div class="dpr-view" data-view="write">
          <div class="dpr-write-c">
            ...
            <div class="dpr-write-actions">
              <button class="dpr-btn dpr-btn-sec" id="dpr-save">${ICO.save} 保存</button>
              <button class="dpr-btn dpr-btn-ai" id="dpr-saveai">${ICO.send} 保存并发送给咨询师</button>
            </div>
          </div>
        </div>
      ...
      <div class="dpr-tabbar" id="dpr-tabbar">
        <div class="dpr-tab active" data-tab="shelf">${ICO.book}<span>书架</span></div>
        <div class="dpr-tab" data-tab="profile">${ICO.sprout}<span>成长档案</span></div>
        <div class="dpr-tab" data-tab="settings">${ICO.settings}<span>设置</span></div>
      </div>`;
    container.appendChild(root);

    // 应用字体
    const fontCfg=await getFontConfig(roche);
    applyFont(root,fontCfg);
    ...
    const homeBtn=root.querySelector("#dpr-home");
    ...
    homeBtn.onclick=()=>{if(roche.exit)roche.exit();else if(window.history.length>1)window.history.back();};
    ...
    function openImageViewer(src){const v=document.createElement("div");v.className="dpr-img-viewer";v.innerHTML='<button class="dpr-img-viewer-close">'+ICO.close+'</button><img src="'+src+'" alt="">';v.onclick=(e)=>{if(e.target===v||e.target.closest(".dpr-img-viewer-close"))v.remove();};root.appendChild(v);}
    ...
    async function renderShelf() {
      ...
      if(years.length===0){
        c.innerHTML='<div class="dpr-shelf-intro"><h2>我的心理日记</h2><p>这里是一个属于你的安全空间。<br>写下心事，选择是否分享给AI咨询师，<br>让每一个感受都被温柔地看见。</p></div><div class="dpr-empty-shelf"><div class="dpr-empty-shelf-icon">'+ICO.book+'</div><p>你的日记书架空空如也</p><p style="font-size:12px;">点击右下角的笔，开始写第一篇日记吧</p></div>';
        return;
      }
    ...
    async function renderBook() {
      ...
      hdr.innerHTML='<div class="dpr-book-hdr-info"><div class="dpr-book-hdr-y">'+curYear+' 年</div><div class="dpr-book-hdr-s">共 '+es.length+' 篇日记</div></div><button class="dpr-btn dpr-btn-sec" style="padding:8px 14px;font-size:13px;" id="dpr-bw">'+ICO.write+' 写日记</button>';
      hdr.querySelector("#dpr-bw").onclick=()=>{isEditing=false;curEntryId=null;showView("write");};
      if(!es.length){list.innerHTML='<div class="dpr-empty">这一年还没有日记<br>点击上方写日记开始记录</div>';return;}
      let h="";
      es.forEach(e=>{
        const m=moodByKey(e.mood);
        const ws=e.weather?(ICO[e.weather.iconKey||"cloud"]||ICO.cloud)+" "+(e.weather.desc||"")+(e.weather.temp!==undefined?" "+e.weather.temp+"°":""):"";
        const imgs=e.images||[];
        const title=e.title?escapeHtml(e.title):'<span style="color:var(--sub);font-weight:400;">无标题</span>';
        h+='<div class="dpr-entry-card" data-id="'+e.id+'">'+(e.aiRead?'<div class="dpr-read-stamp">已阅</div>':'')+'<div class="dpr-entry-top"><span class="dpr-entry-mood" style="--mood-c:'+m.color+';color:'+m.color+';">'+(ICO[m.icon]||ICO.smile)+'</span><div><div class="dpr-entry-date">'+fmtDate(e.timestamp).slice(5)+'</div><div class="dpr-entry-time">'+fmtTime(e.timestamp)+'</div></div>'+(ws?'<span class="dpr-entry-weather">'+ws+'</span>':"")+'</div><div class="dpr-entry-title">'+title+'</div><div class="dpr-entry-preview">'+escapeHtml(e.content.slice(0,120))+'</div>'+(imgs.length?'<div class="dpr-entry-img-count">'+ICO.image+' '+imgs.length+' 张图片</div>':"")+'<div class="dpr-entry-footer"><span class="dpr-badge '+(e.aiRead?'read':'unread')+'">'+(e.aiRead?ICO.check+' 已阅':'未发送')+'</span>'+(e.aiResponse?'<span class="dpr-badge response">'+ICO.chat+' 有回复</span>':"")+'</div></div>';
      });
      list.innerHTML=h;
    ...
    async function renderWrite() {
      const now=nowTs();
      root.querySelector("#dpr-wmeta").innerHTML='<div class="dpr-write-date">'+ICO.calendar+' '+fmtDateCN(now)+'</div><div class="dpr-write-time">'+ICO.clock+' '+fmtTime(now)+'</div><input type="text" class="dpr-title-input" id="dpr-title" placeholder="给这篇日记起个标题…" maxlength="50">';
      writeMood=null;writeTitle="";
      const mr=root.querySelector("#dpr-mood-row");
      mr.innerHTML=MOODS.map(m=>'<div class="dpr-mood-chip" data-mood="'+m.key+'" style="--mood-c:'+m.color+';"><span class="dpr-mood-chip-icon">'+(ICO[m.icon]||ICO.smile)+'</span><span class="dpr-mood-chip-label">'+m.label+'</span></div>').join("");
      ...
      wd.innerHTML='<div class="dpr-weather-icon">'+ICO.loading+'</div><div class="dpr-weather-info"><div class="dpr-weather-desc">正在获取天气…</div></div>';
      wm.innerHTML="";
      MANUAL_WEATHERS.forEach(w=>{const b=document.createElement("div");b.className="dpr-weather-manual-btn";b.innerHTML=(ICO[w.iconKey]||ICO.cloud)+' '+w.desc;b.onclick=()=>{wm.querySelectorAll(".dpr-weather-manual-btn").forEach(x=>x.classList.remove("selected"));b.classList.add("selected");writeWeather={iconKey:w.iconKey,desc:w.desc,temp:null,source:"manual"};wd.innerHTML='<div class="dpr-weather-icon">'+(ICO[w.iconKey]||ICO.cloud)+'</div><div class="dpr-weather-info"><div class="dpr-weather-desc">'+w.desc+'</div><div class="dpr-weather-temp">手动选择</div></div>';};wm.appendChild(b);});

      const aw=await fetchWeather();
      if(aw){writeWeather=aw;renderWeatherDisplay(wd,aw);}
      else{wd.innerHTML='<div class="dpr-weather-icon">'+ICO["cloud-sun"]+'</div><div class="dpr-weather-info"><div class="dpr-weather-desc">无法自动获取</div><div class="dpr-weather-temp">请从下方手动选择</div></div>';}

      renderImgBar();
      const ta=root.querySelector("#dpr-ta");
      const titleInput=root.querySelector("#dpr-title");
      ta.addEventListener("input",()=>updateTokenBar(ta.value));
      if(titleInput)titleInput.addEventListener("input",()=>{writeTitle=titleInput.value;});

      if(isEditing&&editingEntry){
        ta.value=editingEntry.content||"";
        if(titleInput)titleInput.value=editingEntry.title||"";
        writeTitle=editingEntry.title||"";
      ...
      updateTokenBar(ta.value);
      ta.focus();
    }

    function renderWeatherDisplay(el,w){
      el.innerHTML='<div class="dpr-weather-icon">'+(ICO[w.iconKey]||ICO.cloud)+'</div><div class="dpr-weather-info"><div class="dpr-weather-desc">'+w.desc+(w.temp!==null&&w.temp!==undefined?" "+w.temp+"°C":"")+'</div><div class="dpr-weather-temp">'+(w.city?w.city+" · ":"")+'自动定位</div></div><button class="dpr-weather-refresh" id="dpr-wrefresh">'+ICO.refresh+'刷新</button>';
      const b=el.querySelector("#dpr-wrefresh");
      if(b)b.onclick=async()=>{b.textContent="获取中…";const nw=await fetchWeather();if(nw){writeWeather=nw;renderWeatherDisplay(el,nw);}else{toast("天气获取失败，请手动选择");b.innerHTML=ICO.refresh+"刷新";};};
    }

    function renderImgBar() {
      const bar=root.querySelector("#dpr-img-bar");
      let h="";
      writeImages.forEach((src,i)=>{h+='<div class="dpr-img-thumb"><img src="'+src+'" alt=""><button class="dpr-img-thumb-del" data-idx="'+i+'">'+ICO.close+'</button></div>';});
      h+='<div class="dpr-img-add" id="dpr-img-add">'+ICO.image+'<span>添加</span></div>';
      bar.innerHTML=h;
    ...
    async function addImage() {
      ...
      input.onchange=async(e)=>{
        ...
        try{toast("压缩图片中…");const b64=await fileToCompressedBase64(file,1024,0.7);writeImages.push(b64);renderImgBar();updateTokenBar(root.querySelector("#dpr-ta").value);}catch(err){toast("图片加载失败");}
      };
      input.click();
    }

    function updateTokenBar(text) {
      const bar=root.querySelector("#dpr-token-bar");if(!bar)return;
      ...
      const cls=grand>8000?"dpr-token-warn":"dpr-token-ok";
      bar.innerHTML='<span class="'+cls+'">'+ICO.gauge+' 估算 '+fmtTokens(grand)+'</span><span style="color:var(--border);">|</span><span>正文 '+textTokens+' · 图片 '+imgTokens+' · 系统 ~'+sysTokens+'</span>'+(writeImages.length?'<span style="color:var(--border);">|</span><span>'+ICO.image+' '+writeImages.length+' 张</span>':"");
    }

    /* ===== 保存 ===== */
    async function saveEntry(sendAI) {
      const ta=root.querySelector("#dpr-ta");
      const titleEl=root.querySelector("#dpr-title");
      ...
      if(isEditing&&editingEntry){
        editingEntry.title=title;editingEntry.content=content;editingEntry.mood=writeMood;editingEntry.weather=writeWeather;
      ...
      const entry={id:uuid(),timestamp:now,title:title,content:content,mood:writeMood,weather:writeWeather,images:[...writeImages],aiRead:false,aiResponse:null,aiResponseAt:null,createdAt:now};
      if(!data[yk])data[yk]=[];
      data[yk].push(entry);
      await roche.storage.set(K_DIARY,data);
    ...
    async function renderDetail(){
      ...
      const mood=moodByKey(entry.mood);
      const ws=entry.weather?(ICO[entry.weather.iconKey||"cloud"]||ICO.cloud)+" "+(entry.weather.desc||"")+(entry.weather.temp!==null&&entry.weather.temp!==undefined?" "+entry.weather.temp+"°C":""):"";
      const imgs=entry.images||[];
      let h="";
      if(containsCrisisKeyword(entry.content))h+='<div class="dpr-crisis">'+escapeHtml(CRISIS_MESSAGE)+'</div>';
      h+='<div class="dpr-detail-card">'+(entry.aiRead?'<div class="dpr-read-stamp">已阅</div>':'')+'<div class="dpr-detail-hdr"><span class="dpr-detail-mood" style="--mood-c:'+mood.color+';color:'+mood.color+';">'+(ICO[mood.icon]||ICO.smile)+'</span><div><div class="dpr-detail-title">'+escapeHtml(entry.title||"无标题")+'</div><div class="dpr-detail-date">'+fmtDateCN(entry.timestamp)+'</div><div class="dpr-detail-time">'+fmtTime(entry.timestamp)+' · '+mood.label+'</div></div>'+(ws?'<div class="dpr-detail-wtag">'+ws+'</div>':"")+'</div><div class="dpr-detail-content">'+escapeHtml(entry.content)+'</div>';
      ...
      if(!entry.aiRead)h+='<button class="dpr-btn dpr-btn-ai" id="dpr-dsend">'+ICO.send+' 发送给咨询师</button>';
      h+='<button class="dpr-btn dpr-btn-sec" id="dpr-dedit" style="flex:1;">'+ICO.edit+' 编辑</button><button class="dpr-btn dpr-btn-sec" id="dpr-ddel" style="color:var(--danger);flex:1;">'+ICO.trash+' 删除</button></div></div>';
      if(entry.aiResponse)h+='<div class="dpr-ai-card"><div class="dpr-ai-hdr"><div class="dpr-ai-avatar">'+ICO.heart+'</div><div><div class="dpr-ai-name">AI 咨询师</div><div class="dpr-ai-time">'+(entry.aiResponseAt?fmtDate(entry.aiResponseAt)+" "+fmtTime(entry.aiResponseAt):"")+'</div></div></div><div class="dpr-ai-content">'+escapeHtml(entry.aiResponse)+'</div></div>';
      c.innerHTML=h;
    ...
    async function renderProfile(){
      ...
      let mth="";
      if(recent.length){mth='<div class="dpr-mood-track">';recent.forEach((e,i)=>{const m=moodByKey(e.mood);const bh=20+((i*7+13)%35);mth+='<div class="dpr-mood-dot"><span class="dpr-mood-dot-icon" style="color:'+m.color+';">'+(ICO[m.icon]||ICO.smile)+'</span><div class="dpr-mood-dot-bar" style="height:'+bh+'px;background:'+m.color+';opacity:0.6;"></div></div>';});mth+="</div>";}
      else mth='<div class="dpr-empty" style="padding:16px;">还没有日记记录</div>';
      const obs=(profile.observations||[]).slice(-15).reverse();
      c.innerHTML='<div class="dpr-profile-hero"><div class="dpr-profile-hero-icon">'+ICO.sprout+'</div><div class="dpr-profile-hero-title">成长档案</div><div class="dpr-profile-hero-sub">'+(profile.lastUpdated?"最后更新："+fmtDate(profile.lastUpdated):"等待你的第一篇日记")+'</div></div>'+
      '<div class="dpr-pcard"><div class="dpr-pcard-title">'+ICO.chart+' 心情轨迹</div>'+mth+'</div>'+
      '<div class="dpr-pcard"><div class="dpr-pcard-title">'+ICO.wave+' 情绪模式</div><div class="dpr-pcard-body">'+escapeHtml(profile.emotion||"暂在梳理中…")+'</div></div>'+
      '<div class="dpr-pcard"><div class="dpr-pcard-title">'+ICO.brain+' 认知信念</div><div class="dpr-pcard-body">'+escapeHtml(profile.cognition||"暂在观察中…")+'</div></div>'+
      '<div class="dpr-pcard"><div class="dpr-pcard-title">'+ICO.users+' 人际模式</div><div class="dpr-pcard-body">'+escapeHtml(profile.relationship||"暂在观察中…")+'</div></div>'+
      '<div class="dpr-pcard"><div class="dpr-pcard-title">'+ICO.dumbbell+' 应对资源</div><div class="dpr-pcard-body">'+escapeHtml(profile.resources||"暂在挖掘中…")+'</div></div>'+
      '<div class="dpr-pcard"><div class="dpr-pcard-title">'+ICO.trending+' 成长轨迹</div><div class="dpr-pcard-body">'+escapeHtml(profile.growth||"刚刚开始…")+'</div></div>'+
      '<div class="dpr-pcard" style="margin-top:4px;"><div class="dpr-pcard-title">'+ICO.note+' 咨询师观察记录</div></div>'+
    ...
    async function renderSettings(){
      ...
      PROMPT_CATEGORIES.forEach((cat,ci)=>{
        ...
        cat.items.forEach(item=>{
          items+='<div class="dpr-prompt-item"><div class="dpr-prompt-item-label">'+escapeHtml(item.label)+'</div><div class="dpr-prompt-item-note">'+escapeHtml(item.note)+'</div><textarea class="dpr-prompt-ta" data-pk="'+item.key+'" data-def="'+escapeHtml(item.default)+'">'+escapeHtml(prompts[item.key]||item.default)+'</textarea><div class="dpr-prompt-item-act"><button class="dpr-prompt-reset" data-rk="'+item.key+'">'+ICO.reset+' 恢复默认</button></div></div>';
        });
        ph+='<div class="dpr-prompt-cat"><div class="dpr-prompt-cat-hdr" data-ci="'+ci+'"><span class="dpr-prompt-cat-icon">'+(ICO[cat.iconKey]||ICO.send)+'</span><span class="dpr-prompt-cat-title">'+escapeHtml(cat.title)+'</span><span class="dpr-prompt-cat-chev">'+ICO.chevron+'</span></div><div class="dpr-prompt-cat-desc">'+escapeHtml(cat.desc)+'</div><div class="dpr-prompt-cat-body" data-cb="'+ci+'">'+items+'</div></div>';
      });

      const fontPreviewText=fontCfg.type==="url"||fontCfg.type==="base64"?"自定义字体预览 The quick brown fox":"默认系统字体预览 The quick brown fox";
      const fontPreviewStyle=fontCfg.type==="url"||fontCfg.type==="base64"?" style=\"font-family:var(--diary-font);\"":"";

      c.innerHTML=
        '<div class="dpr-set-card"><div class="dpr-set-title">'+ICO.type+' 日记字体</div><div class="dpr-api-hint">自定义日记正文的字体。可选择默认、本地字体文件或字体链接。</div>'+
        '<div class="dpr-font-preview"'+fontPreviewStyle+'>'+fontPreviewText+'</div>'+
        '<div class="dpr-font-type-row">'+
        '<button class="dpr-font-type-btn'+(fontCfg.type==="default"?" selected":"")+'" data-ft="default">默认</button>'+
        '<button class="dpr-font-type-btn'+(fontCfg.type==="url"?" selected":"")+'" data-ft="url">'+ICO.link+' 字体链接</button>'+
        '<button class="dpr-font-type-btn'+(fontCfg.type==="base64"?" selected":"")+'" data-ft="base64">'+ICO.upload+' 本地上传</button>'+
        '</div>'+
        '<div id="dpr-font-url-c" style="display:'+(fontCfg.type==="url"?"block":"none")+';"><div class="dpr-api-fg"><label class="dpr-api-lbl">字体文件 URL (woff2/woff/ttf/otf)</label><input type="text" class="dpr-api-input" id="dpr-font-url" placeholder="https://example.com/font.woff2" value="'+escapeHtml(fontCfg.type==="url"?fontCfg.value:"")+'"></div></div>'+
        '<div id="dpr-font-b64-c" style="display:'+(fontCfg.type==="base64"?"block":"none")+';"><div class="dpr-api-fg"><label class="dpr-api-lbl">上传本地字体文件 (ttf/otf/woff/woff2)</label><button class="dpr-api-fetch" id="dpr-font-upload" style="width:100%;">'+ICO.upload+' 选择字体文件</button><div class="dpr-api-status" id="dpr-font-status">'+(fontCfg.type==="base64"?"已加载本地字体":"")+'</div></div></div>'+
        '<button class="dpr-btn dpr-btn-pri" style="margin-top:8px;padding:8px 14px;font-size:13px;" id="dpr-save-font">'+ICO.save+' 保存字体设置</button></div>'+

        '<div class="dpr-set-card"><div class="dpr-set-title">'+ICO.edit+' 提示词编辑</div><div class="dpr-api-hint">以下提示词在对应功能中使用。默认已填充推荐内容，可直接编辑。点击分类展开/收起。</div><div style="display:flex;gap:8px;margin-top:8px;"><button class="dpr-btn dpr-btn-pri" style="flex:1;padding:8px 14px;font-size:13px;" id="dpr-sp">'+ICO.save+' 保存全部提示词</button><button class="dpr-btn dpr-btn-sec" style="padding:8px 14px;font-size:13px;" id="dpr-rp">全部恢复默认</button></div></div>'+
        ph+
        '<div class="dpr-set-card"><div class="dpr-set-title">'+ICO.settings+' 独立 API 配置</div><div class="dpr-api-hint">配置后插件使用此独立 API 调用 AI。留空则使用 Roche 全局默认。兼容 OpenAI 格式 /v1 端点。</div>'+
        ...
        '<div class="dpr-api-fg"><label class="dpr-api-lbl">Model</label><div style="display:flex;gap:8px;align-items:flex-start;"><input type="text" class="dpr-api-input" id="dpr-am" placeholder="gpt-4o" value="'+escapeHtml(apiCfg.model||"")+'" style="flex:1;"><button class="dpr-api-fetch" id="dpr-af">'+ICO.cloud+' 拉取模型</button></div><select class="dpr-api-select" id="dpr-ams" style="display:none;margin-top:8px;"><option value="">-- 拉取后选择 --</option></select><div class="dpr-api-status" id="dpr-ast"></div></div>'+
        '<div style="display:flex;gap:8px;margin-top:8px;"><button class="dpr-btn dpr-btn-pri" style="flex:1;padding:8px 14px;font-size:13px;" id="dpr-sa">'+ICO.save+' 保存 API</button><button class="dpr-btn dpr-btn-sec" style="padding:8px 14px;font-size:13px;" id="dpr-ca">清空</button></div></div>'+

        '<div class="dpr-set-card"><div class="dpr-set-title">关于</div><div class="dpr-about"><p><b>心理咨询室·日记 v3.1.0</b></p><p>一本属于你的心理成长日记。</p><p>· 一年一本日记本，书架式管理<br>· 写日记自动添加日期时间<br>· 可选心情，自动获取天气<br>· 可上传图片到日记<br>· 日记标题，自定义字体<br>· Token 估算实时显示<br>· 可选发送给AI咨询师（支持图片）<br>· 已阅日记显示印章<br>· AI维护专业成长档案<br>· 分类提示词可自定义<br>· 独立API配置与模型拉取</p><p style="margin-top:10px;font-size:11px;opacity:0.7;">本应用非医疗诊断工具，不能替代专业心理咨询/精神科诊疗。如遇紧急情况，请拨打全国心理援助热线 12356。</p></div></div>'+

        '<div class="dpr-set-card"><div class="dpr-set-title">数据管理</div><button class="dpr-danger-btn" id="dpr-clear">清空全部数据</button></div>';

      /* 字体类型切换 */
      let pendingFontType=fontCfg.type||"default";
      let pendingFontValue=fontCfg.value||"";
      let pendingFontFormat=fontCfg.format||"truetype";
      c.querySelectorAll(".dpr-font-type-btn").forEach(btn=>{
        btn.onclick=()=>{
          pendingFontType=btn.dataset.ft;
          c.querySelectorAll(".dpr-font-type-btn").forEach(b=>b.classList.remove("selected"));
          btn.classList.add("selected");
          c.querySelector("#dpr-font-url-c").style.display=pendingFontType==="url"?"block":"none";
          c.querySelector("#dpr-font-b64-c").style.display=pendingFontType==="base64"?"block":"none";
        };
      });

      /* 字体上传 */
      const uploadBtn=root.querySelector("#dpr-font-upload");
      if(uploadBtn)uploadBtn.onclick=()=>{
        const input=document.createElement("input");
        input.type="file";input.accept=".ttf,.otf,.woff,.woff2,font/*";
        input.onchange=async(e)=>{
          const file=e.target.files[0];if(!file)return;
          const statusEl=root.querySelector("#dpr-font-status");
          statusEl.className="dpr-api-status";statusEl.textContent="正在加载字体…";
          try{
            const reader=new FileReader();
            reader.onload=()=>{
              const b64=reader.result;
              pendingFontValue=b64;
              const ext=file.name.split(".").pop().toLowerCase();
              pendingFontFormat=ext==="woff2"?"woff2":ext==="woff"?"woff":ext==="otf"?"opentype":"truetype";
              statusEl.className="dpr-api-status ok";statusEl.textContent="已加载: "+file.name;
            };
            reader.readAsDataURL(file);
          }catch(err){statusEl.className="dpr-api-status err";statusEl.textContent="字体加载失败";}
        };
        input.click();
      };

      /* 保存字体 */
      root.querySelector("#dpr-save-font").onclick=async()=>{
        let cfg={type:pendingFontType};
        if(pendingFontType==="url"){
          const urlVal=root.querySelector("#dpr-font-url").value.trim();
          if(!urlVal){toast("请填写字体链接");return;}
          cfg.value=urlVal;
        } else if(pendingFontType==="base64"){
          if(!pendingFontValue){toast("请先上传字体文件");return;}
          cfg.value=pendingFontValue;cfg.format=pendingFontFormat;
        }
        await roche.storage.set(K_FONT_CONFIG,cfg);
        applyFont(root,cfg);
        toast("字体设置已保存");
        renderSettings();
      };
      ...
      root.querySelector("#dpr-af").onclick=async()=>{
        ...
        fb.disabled=false;fb.innerHTML=ICO.cloud+" 拉取模型";
      ...
      root.querySelector("#dpr-clear").onclick=async()=>{const ok=await roche.ui.confirm({title:"清空全部数据",message:"将删除所有日记、成长档案、提示词、字体和API设置，是否继续？"}).catch(()=>window.confirm("将删除所有数据，是否继续？"));if(!ok)return;await Promise.all([roche.storage.delete(K_DIARY),roche.storage.delete(K_PROFILE),roche.storage.delete(K_PROMPTS),roche.storage.delete(K_API_CONFIG),roche.storage.delete(K_FONT_CONFIG)]);toast("已清空全部数据");showView("shelf");};
    }
  ...
  async function unmount(container) {
    ...
    const f=document.getElementById("dpr-custom-font");if(f)f.remove();
    container.replaceChildren();
  }

  window.RochePlugin.register({
    id:PLUGIN_ID, name:"心理咨询室·日记", version:"3.1.0",
